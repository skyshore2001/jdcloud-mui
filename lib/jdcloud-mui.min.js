...
{if(m_curState==null)
return;setHash(m_curState.pageRef,url);}
...
jpage.find("style:not([mui-nofix])").each(function(){$(this).html(selfCtx.fixPageCss($(this).html(),"#"+pageId));});jpage.find("style").attr("mui-origin",pageId).appendTo(document.head);jpage.attr("id",pageId).addClass("mui-page").hide().appendTo(self.container);var val=jpage.attr("mui-script");if(val!=null){if(path==null)
...
$(window).on('popstate',function(ev){m_curState=ev.originalEvent.state;showPage();});$(window).on('orientationchange',fixPageSize);$(window).on('resize',fixPageSize);function fixPageSize()
...
{return m_toPageId;}
self.m_enhanceFn={};self.enhanceWithin=enhanceWithin;function enhanceWithin(jp)
...
return opt;}
$(document).on("pagecreate",function(ev){var jpage=$(ev.target);enhanceWithin(jpage);});self.m_enhanceFn["#footer"]=enhanceFooter;self.m_enhanceFn[".mui-navbar"]=enhanceNavbar;function activateElem(jo)
...
{var ref=jo.attr("mui-linkto");if(ref){var jlink=self.activePage.find(ref);jlink.toggle(active);jlink.toggleClass("active",active);}}}
...
var opt=self.getOptions(jdlg);if(opt.type==null){jdlg.find("#btnOK, #btnCancel").click(app_alert_click);}
opt.type=type;opt.fn=fn;opt.alertOpt=alertOpt;opt.isClone=isClone;jdlg.find("#btnCancel").toggle(type=="q"||type=="p");var jtxt=jdlg.find("#txtInput");jtxt.toggle(type=="p");if(type=="p"){jtxt.val(alertOpt.defValue);}
...
var btnId=this.id;if(opt.fn&&btnId=="btnOK"){var param;if(opt.type=="p"){param=jdlg.find("#txtInput").val();}
opt.fn(param);}
...
self.container=$(document.body);enhanceWithin(self.container);self.container.trigger("muiInit");if(self.showFirstPage)
showPage();}
$(main);}
...
pageRef=MUI.options.homePage;if(m_allowedEntries==null||m_allowedEntries.indexOf("#"+MUI.activePage.attr("id"))>=0)
...
if(firstShow_){showOrderList(false,false);}}
...
initPageList.options={navRef:">.hd .mui-navbar",listRef:">.bd .p-list",pageszName:"_pagesz",pagekeyName:"_pagekey",canPullDown:true,onRemoveAll:function(jlst){jlst.empty();}};}
...
return itf;}}
jdModule("jdcloud.mui.name",JdcloudMuiName);function JdcloudMuiName()
{var self=this;var mCommon=jdModule("jdcloud.common");window.MUI=jdModule("jdcloud.mui");$.extend(MUI,mCommon);$.each(["intSort","numberSort","callSvr","callSvrSync","app_alert",],function(){window[this]=MUI[this];});}