...
{var self=this;self.assert(window.jQuery,"require jquery lib.");var mCommon=jdModule("jdcloud.common");self.getFormData=getFormData;function getFormData(jo,doGetAll)
...
return true;}
return val;},setDisabled:function(val){var jo=this.ji;jo.prop("disabled",!!val);if(val)
...
return o.selectedIndex>=0?o.options[o.selectedIndex].innerText:'';return this.getValue(jo);},getTitle:function(){return this.ji.closest("td").prev("td").html();},setFocus:function(){var j1=this.getShowbox();if(j1.is(":hidden")){selectTab(j1);}
...
return this.getReadonly();this.setReadonly(v);return this;},visible:function(v){var jp=this.ji.closest("tr,.wui-field");if(v===undefined){return this.ji.css("display")!="none"&&jp.css("display")!="none";}
...
{var fn=window.onerror;window.onerror=function(msg,script,line,col,errObj){if($.active||self.isBusy){setTimeout(function(){$.active=0;self.isBusy=0;self.hideLoading();},1000);}
if(fn&&fn.apply(this,arguments)===true)
return true;if(errObj instanceof DirectReturn||/abort$/.test(msg))
...
params.XDEBUG_SESSION_START=1;var ret=mCommon.appendParam(url,$.param(params));return makeUrlObj(ret);function makeUrlObj(url)
...
jpage.css("z-index",1).show();self.container.show();m_isback=null;self.activePage=jpage;fixPageSize();var title=jpage.find(".hd h1, .hd h2").filter(":first").text()||self.title||jpage.attr("id");setDocTitle(title);if(!enableAni){onAnimationEnd();}
...
{jfooter.remove();enhanceNavbar(jfooter);jfooter.addClass("ft").addClass("mui-navbar");var jnavs=jfooter.find(">a");var id2nav=null;function getNav(pageId){if(id2nav==null){id2nav={};jnavs.each(function(i,e){if(e.style.display=="none")
...
var s={i:"提示",w:"警告",e:"出错了",q:"确认",p:"输入"}[type];var jdlg=self.container.find("#muiAlert");if(jdlg.size()==0){var html=''+'<div id="muiAlert" class="mui-dialog">'+' <div class="hd p-title"></div>'+' <div class="bd">'+'  <div class="p-msg"></div>'+'  <input type="text" id="txtInput" style="border:1px solid #bbb; height:30px; text-align: center">'+' </div>'+' <div class="ft">'+'  <a href="javascript:;" id="btnCancel" class="mui-btn">取消</a>'+'  <a href="javascript:;" id="btnOK" class="mui-btn primary">确定</a>'+' </div>'+'</div>'
...
{if(self.options.enableSwitchApp&&(history.length<=1||g_args.enableSwitchApp)){var appPage0=mCommon.getStorage("appPage")
...
else{g_cordova=parseInt(g_args.cordova||mCommon.getStorage("cordova"));g_args.cordova=g_cordova;mCommon.setStorage("cordova",g_cordova);$(function(){var path='./';if(/iPhone|iPad|Macintosh/i.test(navigator.userAgent)){if(g_args.mergeJs){mCommon.loadScript(path+"lib-cordova-ios.min.js");}
...
window.Plugins={exists:function(pname){return plugins_[pname]!==undefined;},list:function(){return plugins_;}};function switchTestMode()
...
return;var diff=mCommon.getTimeDiffDscr(tm,new Date());var str="<p>下拉可以刷新</p>";var str1="<p>上次刷新："+diff+"</p>";if(uptoThreshold){str="<p>松开立即刷新</p>";msg="<div>"+str+str1+"</div>";}
