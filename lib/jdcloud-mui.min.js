...
return;if($.isArray(v)){if(v[0])
condArr.push(k+">='"+v[0]+"'");if(v[1])
condArr.push(k+"<'"+v[1]+"'");return;}
var arr=v.toString().split(/\s+(and|or)\s+/i);var str='';var bracket=false;var isTm=/(Tm|^tm)\d*$/.test(k);var isDt=/(Dt|^dt)\d*$/.test(k);$.each(arr,function(i,v1){if((i%2)==1){str+=' '+v1.toUpperCase()+' ';bracket=true;return;}
...
{var self=this;var mCommon=jdModule("jdcloud.common");self.lastError=null;var m_tmBusy;var m_manualBusy=0;var m_appVer;var m_silentCall=0;self.disableBatch=false;var m_curBatch=null;self.m_curBatch=m_curBatch;var RV_ABORT=undefined;self.mockData={};var ajaxOpt={beforeSend:function(xhr){this.xhr_=xhr;},dataFilter:function(data,type){if(this.jdFilter!==false&&(type=="json"||type=="text")){rv=defDataProc.call(this,data);if(rv!==RV_ABORT)
