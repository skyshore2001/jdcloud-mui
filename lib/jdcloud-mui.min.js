...
data={};var jo1=jo.filter("[name]:not([noReset])");jo.find("[name]:not([noReset])").add(jo1).each(function(){var ji=$(this);var name=ji.attr("name");var content=data[name];if(opt1.setOnlyDefined&&content===undefined)
return;var isInput=ji.is(":input");if(content===undefined){if(isInput){if(ji[0].tagName==="TEXTAREA")
content=ji.html();else
content=ji.attr("value");if(content===undefined)
content="";}
else{content="";}}
if(isInput){ji.val(content);}
...
var showPageOpt_=$.extend({ani:self.options.ani},opt,self.nextShowPageOpt);self.nextShowPageOpt=null;var ret=handlePageStack(pageRef);if(ret===false)
...
if(!skipBeforeShow)
jpage.trigger("pagebeforeshow",[showPageOpt_]);if(toPageId!=m_toPageId)
...
$(document).on("pagebeforeshow",function(ev){var jpage=$(ev.target);var pageId=jpage.attr("id");if(m_toPageId!=pageId)
...
return;var queryParam={ac:jf.attr("action")};if(opt.validate){if(false===opt.validate(jf,queryParam))
return false;}
...
if(mCommon.isIOS()){bar.overlaysWebView(false);setTimeout(function(){$(window).trigger("resize");});}}});function isLoginPage(pageRef)
...
return;m_allowedEntries=allowedEntries;if(location.hash&&location.hash!="#"&&allowedEntries.indexOf(location.hash)<0){location.href=location.pathname;self.app_abort();}}
function parseArgs()
{if(location.search)
...
else{g_cordova=parseInt(g_args.cordova||mCommon.getStorage("cordova"));g_args.cordova=g_cordova;mCommon.setStorage("cordova",g_cordova);$(function(){var path='./';if(mCommon.isIOS()){mCommon.loadScript(path+"cordova-ios/cordova.js?__HASH__,..");if(!m_opt.disableFastClick){mCommon.loadScript(path+"lib/fastclick.min.js").then(function(){Origami.fastclick(document.body);});}}
