...
return;return jo.attr("name")||jo.prop("name");},getDisabled:function(jo){var v=jo.prop("disabled")||jo.attr("disabled");var o=jo[0];if(!v&&o.tagName=="INPUT"){if(o.type=="radio"&&!o.checked)
return true;}
return v;},setDisabled:function(jo,val){jo.prop("disabled",!!val);if(val)
jo.attr("disabled","disabled");else
jo.removeAttr("disabled");},setValue:function(jo,val){var isInput=jo.is(":input");if(val===undefined){if(isInput){var o=jo[0];if(o.tagName==="TEXTAREA")
...
return val;},getShowbox:function(jo){return jo;}};function formItems(jo,cb)
...
{var optStr=jo.attr("data-options");var opts;try{if(optStr!=null){if(optStr.indexOf(":")>0){opts=eval("({"+optStr+"})");}
...
return;jo.each(function(i,e){var je=$(e);var enhanced=je.data("mui-enhanced");if(enhanced)
return;je.data("mui-enhanced",true);fn(je);});});}
...
if(v==="empty")
v="";if(v.length==0||v.match(/\D/)||v[0]=='0'){v=v.replace(/'/g,"\\'");if(self.options.fuzzyMatch&&op=="="&&v.length>0){op=" like ";v="%"+v+"%";}
return op+"'"+v+"'";}
return op+v;}
...
function handleOne(k,v){if(v==null||v===""||($.isArray(v)&&v.length==0))
...
if(!opt.contentType&&opt.data&&$.isPlainObject(opt.data)){$.each(opt.data,function(i,e){if(typeof(e)=="object"){opt.contentType="application/json";return false;}})}
...
{if(!g_cordova)
return;$(document).on("pageshow",function(){var color=MUI.activePage.find(".hd").css("backgroundColor");if(color){var colorHex=self.rgb2hex(color);setTopbarColor(colorHex);}});}
...
{var pageRef=getPageRef(page);m_onLoginOK=function(){if(MUI.activePage&&isLoginPage(MUI.getToPageId())){if(pageRef==null)
...
function isAllowed(){var found=false;var hash=location.hash;$.each(allowedEntries,function(){if((this instanceof RegExp&&this.test(hash))||this==hash){found=true;return false;}});return found;}}
function parseArgs()
{if(location.search)
...
else{g_cordova=parseInt(g_args.cordova||mCommon.getStorage("cordova"));g_args.cordova=g_cordova;mCommon.setStorage("cordova",g_cordova);$(function(){var path='./';if(mCommon.isIOS()){if(g_args.mergeJs){mCommon.loadScript(path+"lib-cordova-ios.min.js");}
...
jo_.html(msg);jo_.height(height);if(ac=="D"){var c=cont_.getElementsByClassName("mui-pullHint")[0];if(c)
jo_.appendTo(c);else
jo_.prependTo(cont_);}
